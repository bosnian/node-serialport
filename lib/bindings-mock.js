...
MockBindings.prototype.emitData = function(data) {
  ...
  if (this.port.pendingReadCB) {
    var bytes = this.port.pendingReadCB[0];
    var cb = this.port.pendingReadCB[1];
    this.port.pendingReadCB = null;
    processNextTick(this.read.bind(this, bytes, cb));
  }
};
...
MockBindings.prototype.read = function(bytes, cb) {
  if (!this.isOpen) {
    return processNextTick(cb, new ClosedPortError('read'));
  }

  if (this.port.data.length > 0) {
    var data = this.port.data.slice(0, bytes);
    this.port.data = this.port.data.slice(bytes);
    processNextTick(cb, null, data);
  } else {
    this.port.pendingReadCB = [bytes, cb];
  }
};
